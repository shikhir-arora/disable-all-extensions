import { allExtensionInfo, updateIconState, enableExtensions, disableExtensions, debounce } from "../public/utils/functions.js";
...
const handleToggleExtensions = () => {
    chrome.storage.sync.get(["alwaysOn"], ({ alwaysOn = [] }) => {
        chrome.storage.local.get(["isDisablingOtherExts", "lastEnabledExts"], async (data) => {
            const extensionList = await chrome.management.getAll();
            const notWhitelistedExts = extensionList.filter(ext => !alwaysOn.includes(ext.id) && ext.id !== chrome.runtime.id);
            const { enabledExts, disabledExts } = allExtensionInfo(extensionList);

            if (data.isDisablingOtherExts) {
                enableExtensions(data.lastEnabledExts.map(extId => extensionList.find(ext => ext.id === extId)).filter(ext => ext));
                chrome.storage.local.set({ isDisablingOtherExts: false }, updateIconState);
            } else {
                chrome.storage.local.set({ lastEnabledExts: enabledExts.map(ext => ext.id) }, () => {
                    disableExtensions(notWhitelistedExts);
                    chrome.storage.local.set({ isDisablingOtherExts: true }, updateIconState);
                });
            }
        });
    });
...
chrome.tabs.onRemoved.addListener(async (tabId) => {
    if (tabId === isolationTabId) {
        ...
        disableExtensions(allExts.filter(ext => lastDisabledExts.includes(ext.id)));
    }
});
